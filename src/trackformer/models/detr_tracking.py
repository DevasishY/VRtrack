...
class DETRTrackingBase(nn.Module):
    ...
    def add_track_queries_to_targets(
        self, targets, prev_indices, prev_out, add_false_pos=True
    ):
        ...
        for i, (target, prev_ind) in enumerate(zip(targets, prev_indices)):
            ...
            if add_false_pos:
                ...
                prev_boxes_matched = prev_out["pred_boxes"][
                    i, prev_out_ind[target_ind_matching]
                ]

                not_prev_out_ind = torch.arange(prev_out["pred_boxes"].shape[1])
                not_prev_out_ind = [
                    ind.item() for ind in not_prev_out_ind if ind not in prev_out_ind
                ]
                ...
                for prev_box_matched in prev_boxes_matched:
                    ...
                    if random.uniform(0, 1) < self._track_query_false_positive_prob:
                        prev_boxes_unmatched = prev_out["pred_boxes"][
                            i, not_prev_out_ind
                        ]
                        ...
                        prev_box_ious, _ = box_ops.box_iou(
                            box_ops.box_cxcywh_to_xyxy(
                                prev_box_matched.unsqueeze(dim=0)
                            ),
                ...
                prev_out_ind = torch.tensor(
                    prev_out_ind.tolist() + random_false_out_ind
                ).long()

                target_ind_matching = torch.cat(
                    [
                        target_ind_matching,
                        torch.tensor([False] * len(random_false_out_ind))
                        .bool()
                        .to(device),
                    ]
                )
            ...
            target["track_query_boxes"] = prev_out["pred_boxes"][
                i, prev_out_ind
            ].detach()
        ...
        max_track_query_hs_embeds = max(
            [len(t["track_query_hs_embeds"]) for t in targets]
        )
        for i, target in enumerate(targets):
            ...
            if not num_add:
                target["track_queries_placeholder_mask"] = torch.zeros_like(
            ...
            target["track_query_boxes"] = torch.cat(
                [torch.zeros(num_add, 4).to(device), target["track_query_boxes"]]
            )
        ...
        if targets is not None and not self._tracking:
            ...
            with backprop_context():
                if "prev_prev_image" in targets[0]:
                    ...
                    prev_prev_indices = self._matcher(
                        prev_prev_outputs_without_aux, prev_prev_targets
                    )
