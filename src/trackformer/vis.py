...
class LineVis(BaseVis):
    ...
    def plot(self, y_data, x_label):
        ...
        if isinstance(x_label, list):
            ...
            y_data = [
                d.cpu() if torch.is_tensor(d) else torch.tensor(d) for d in y_data
            ]
...
class ImgVis(BaseVis):
    ...
    def plot(self, images):
        ...
        self.win = self.viz.images(images, nrow=1, opts=self.viz_opts, win=self.win)
        self.viz.save([self.viz.env])
...
def vis_results(visualizer, img, result, target, tracking):
    ...
    for box_id in range(len(keep)):
        ...
        axarr[0].add_patch(
            plt.Rectangle(
                (x1, y1), x2 - x1, y2 - y1, fill=False, color=rect_color, linewidth=2
            )
        )

        axarr[0].text(
            x1, y1 + offset, text, fontsize=10, bbox=dict(facecolor="white", alpha=0.5)
        )
    ...
    # Ensure target["track_queries_mask"] and target["track_queries_fal_pos_mask"] are on the same device as keep
    target["track_queries_mask"] = target["track_queries_mask"].to(keep.device)
    target["track_queries_fal_pos_mask"] = target["track_queries_fal_pos_mask"].to(
        keep.device
    )
    ...
    if num_track_queries_with_id != num_track_queries:
        ...
        legend_handles.append(
            mpatches.Patch(color="red", label=track_queries_fal_pos_label)
        )
...
def build_visualizers(args: dict):
    ...
    legend = ["BBOX AP IoU=0.50:0.95", "BBOX AP IoU=0.50", "BBOX AP IoU=0.75"]

    if args.masks:
        legend.extend(["MASK AP IoU=0.50:0.95", "MASK AP IoU=0.50", "MASK AP IoU=0.75"])
    ...
    opts = dict(title="TRAIN EXAMPLE RESULTS", width=2500, height=2500)
